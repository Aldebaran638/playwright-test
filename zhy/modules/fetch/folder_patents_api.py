...
from zhy.modules.fetch.folder_patents_abstract import enrich_page_patents_with_abstracts
from zhy.modules.common.types.folder_patents import AuthRefreshRequiredError, FolderAuthState, TransientRequestError
from zhy.modules.persist.json_io import load_json_file_any_utf, save_json
from zhy.modules.persist.page_path import build_patents_page_path
    ...
    *,
    ...
    for attempt_index in range(attempts):
        try:
            ...
            logger.warning(
                "[folder_patents_api] retry request page={} attempt={}/{} error={} sleep={}s",
                page,
                attempt_index + 1,
                attempts,
                exc,
    ...
    *,
    ...
    while True:
        ...
        for page_number in pages:
            output_path = build_patents_page_path(output_root, space_id, folder_id, page_number)
            if resume and output_path.exists():
                try:
                    ...
                    logger.warning(
                        "[folder_patents_api] resume read failed and will refetch: {}",
                        output_path,
                    )
        ...
        for page_number, parsed in batch_results:
            ...
            for failure in abstract_failures:
                folder_summary["abstract_failures"].append({"page_number": page_number, **failure})
