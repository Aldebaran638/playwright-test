...
from zhy.modules.common.types.folder_patents import FolderAuthState, HybridTaskConfig, strip_or_none
...
async def ensure_logged_in(managed, config: HybridTaskConfig) -> None:
    ...
    logger.info(
        "[folder_patents_auth] ensure_logged_in: cookie_file={} target_home={}",
        config.cookie_file,
    ...
    logger.info("[folder_patents_auth] ensure_logged_in complete: final_url={}", page.url)
    await page.close()
...
async def capture_patents_auth_state(managed, config: HybridTaskConfig, space_id: str, folder_id: str) -> FolderAuthState:
    ...
    logger.info(
        "[folder_patents_auth] capture_auth start: space_id={} folder_id={} start_page={}",
        space_id,
        folder_id,
        config.start_page,
    )
    ...
    try:
        ...
        logger.info(
            "[folder_patents_auth] capture_auth success: folder_id={} request_url={} has_authorization={} has_x_client_id={} has_x_device_id={} has_b3={} has_cookie={}",
            folder_id,
...
async def refresh_auth_state(managed, config: HybridTaskConfig, space_id: str, folder_id: str) -> FolderAuthState:
    ...
    logger.warning(
        "[folder_patents_auth] refreshing auth state: space_id={} folder_id={}",
        space_id,
        folder_id,
    )
