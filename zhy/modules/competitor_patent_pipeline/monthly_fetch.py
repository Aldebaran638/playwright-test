...
from zhy.modules.folder_patents_hybrid.api_fetch import RequestScheduler, post_page_async
from zhy.modules.folder_patents_hybrid.models import AuthRefreshRequiredError, HybridTaskConfig
...
def build_monthly_page_output_payload(parsed: dict, matched_rows: list[dict], source_page_number: int, month_text: str) -> dict:
    ...
    逻辑：保留原始 data 结构，只替换 patents_data 并附加来源页和月份元信息。
    """
...
def filter_folder_items_for_test(folder_items: list[dict], test_folder_ids: list[str]) -> list[dict]:
    ...
    返回值：过滤后的公司列表。
    逻辑：白名单为空时直接返回全部；有值时只保留命中的公司，便于联调阶段缩小范围。
    """
...
def build_auth_capture_config(config: CompetitorPatentPipelineConfig, output_root: Path) -> HybridTaskConfig:
    """简介：把总流程配置映射为鉴权抓取可复用的 HybridTaskConfig。
    参数：config 为总流程配置；output_root 为临时输出目录占位。
    返回值：可供 refresh_auth_state 调用的 HybridTaskConfig。
    逻辑：复用现有 folder_patents_hybrid 的登录和请求截获实现，避免重复造轮子。
    """
    ...
    return HybridTaskConfig(
        ...
        abstract_request_url="",
        abstract_origin="",
        abstract_referer="",
        abstract_x_patsnap_from="",
        abstract_request_template={},
        abstract_text_field_name="",
...
async def fetch_monthly_patents_for_folder(
    *,
    ...
    返回值：单个文件夹抓取 summary。
    逻辑：按 PBD 倒序逐页抓取，整页太新则跳过不保存，整页太旧则立即停止，命中月份时只保存命中记录。
    """
...
async def run_monthly_patent_fetch(
    *,
    ...
    参数：config 为总流程配置；managed 为浏览器上下文包装对象；folder_mapping_file 为已过滤的竞争对手映射文件。
    ...
    auth_config = build_auth_capture_config(config, config.pipeline_output_dir)
    ...
    async def process_single_folder(folder_item: dict) -> dict:
        ...
        while True:
            try:
                summary = await fetch_monthly_patents_for_folder(
                    config=config,
                    scheduler=scheduler,
                ...
                refresh_count += 1
                logger.warning(
                    "[competitor_patent_pipeline] refresh auth for monthly patents: folder_id={} refresh_count={}",
                    folder_id,
                    refresh_count,
                )
                auth_state = await refresh_auth_state(
                ...
                local_auth_headers = auth_state.to_headers(
                    origin=config.workspace_origin,
                    referer=config.workspace_referer,
                    user_agent=config.workspace_user_agent,
                    x_api_version=config.workspace_x_api_version,
                ...
                local_auth_template = auth_state.body_template if isinstance(auth_state.body_template, dict) else {}
        return summary
