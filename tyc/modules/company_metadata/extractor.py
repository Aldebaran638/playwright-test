...
def extract_company_metadata(page: Page, source: str | Path | None = None) -> dict[str, Any]:
    logger.info("[company_metadata.extractor] 开始提取公司详情页元信息")

    run_step(
        page.wait_for_selector,
        "#J_CompanyHeaderContent",
        timeout=15000,
        step_name="等待公司详情主容器加载",
        critical=True,
        retries=1,
    )
    root = page.locator("#J_CompanyHeaderContent")

    data_result = run_step(
        root.evaluate,
        """node => {
        ...
        step_name="提取公司详情页元信息",
        critical=True,
        retries=0,
    )
    if data_result.value is None:
        raise RuntimeError("公司详情页元信息提取结果为空")
    ...
    logger.info("[company_metadata.extractor] 公司详情页元信息提取完成")
    return data
