...
DEFAULT_OUTPUT_DIR = Path("tyc/data/output/korea_law_content")
...
PREFERRED_QUERY_KEYS = ("lsiSeq", "chrClsCd", "ancYd", "ancNo", "joNo", "admRulSeq", "admRulPttninfSeq")
...
def build_output_filename_from_url(url: str) -> str:
    ...
    raw_name = "__".join(name_parts) if name_parts else f"law_go_kr__{short_hash}"
    safe_name = re.sub(r'[<>:"/\\|?*\s]+', "_", raw_name).strip("._")
    if not safe_name:
        safe_name = f"law_go_kr__{short_hash}"
...
def fetch_content_body_text(
        ...
        try:
            with sync_playwright() as playwright:
                ...
                try:
                    ...
                    page.wait_for_selector("#contentBody", state="visible", timeout=wait_timeout_ms)
                    raw_text = page.locator("#contentBody").inner_text(timeout=wait_timeout_ms)
                    text = normalize_text(raw_text)
                    if not text:
                        raise ValueError("The contentBody element was found, but it did not contain text.")
                    return text
                finally:
                    browser.close()
            ...
            last_error = exc
            logger.warning(
                "[korea_law_content_fetcher] attempt "
    ...
    if last_error is None:
        raise RuntimeError(f"Failed to fetch contentBody text from {url}")
    raise last_error


def save_content_body_text(
    text: str,
    source_url: str,
...
def fetch_and_save_content_body(
    url: str,
    output_dir: str | Path = DEFAULT_OUTPUT_DIR,
    output_filename: str | None = None,
    timeout: int = DEFAULT_TIMEOUT_SECONDS,
) -> Path:
    text = fetch_content_body_text(url, timeout=timeout)
    return save_content_body_text(
        text=text,
        source_url=url,
