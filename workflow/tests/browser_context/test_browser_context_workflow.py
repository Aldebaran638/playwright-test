...
class TestBrowserContextWorkflow(unittest.TestCase):
    ...
    def _log_probe_result(mode: str, result: BrowserContextProbeResult) -> BrowserContextProbeResult:
        logger.info(
            "[probe] mode={} success={} failure_reason={} detail={}",
            mode,
            result.success,
    ...
    def test_infer_requested_mode_covers_all_four_cases(self) -> None:
        # 验证首选模式推断覆盖四种输入组合。
        self.assertEqual(
            infer_requested_mode(BrowserContextUserInput("C:/Edge/msedge.exe", "D:/browser_data")),
            "full_persistent",
        )
        self.assertEqual(
    ...
    def test_get_next_mode_follows_expected_downgrade_rules(self) -> None:
        ...
        self.assertEqual(get_next_mode("full_persistent", "user_data_incompatible"), "custom_browser_ephemeral")
        self.assertEqual(get_next_mode("full_persistent", "startup_failed"), "default_browser_ephemeral")
        self.assertEqual(get_next_mode("custom_browser_ephemeral", "browser_unavailable"), "default_browser_ephemeral")
        self.assertEqual(get_next_mode("default_browser_persistent", "user_data_incompatible"), "default_browser_ephemeral")
    ...
    def test_resolve_browser_context_mode_succeeds_without_fallback(self) -> None:
        ...
        def fake_probe(mode: str, normalized_input: BrowserContextUserInput) -> BrowserContextProbeResult:
            ...
            return self._log_probe_result(
                mode,
                BrowserContextProbeResult(mode=mode, success=True, detail="full persistent startup ok"),
            )
    ...
    def test_resolve_browser_context_mode_downgrades_when_browser_is_unavailable(self) -> None:
        ...
        self.assertEqual(result.fallback_chain, ["full_persistent", "default_browser_persistent"])
        self.assertEqual(probe_calls, ["full_persistent", "default_browser_persistent"])

    def test_resolve_browser_context_mode_downgrades_when_user_data_is_unavailable(self) -> None:
        # 验证用户数据目录不可用时，会从第一档降级到第二档。
    ...
    def test_resolve_browser_context_mode_downgrades_to_default_ephemeral_when_user_data_is_incompatible(self) -> None:
        ...
        self.assertEqual(result.fallback_chain, ["default_browser_persistent", "default_browser_ephemeral"])

    def test_resolve_browser_context_mode_returns_failure_when_all_modes_fail(self) -> None:
        # 验证所有模式都失败时，会返回统一失败结果。
        ...
        def fake_probe(mode: str, normalized_input: BrowserContextUserInput) -> BrowserContextProbeResult:
            return self._log_probe_result(
                ...
                BrowserContextProbeResult(
                    mode=mode,
                    success=False,
                    failure_reason="browser_unavailable" if mode == "custom_browser_ephemeral" else "startup_failed",
        ...
        self.assertTrue(result.used_fallback)
        self.assertEqual(result.fallback_chain, ["custom_browser_ephemeral", "default_browser_ephemeral"])
